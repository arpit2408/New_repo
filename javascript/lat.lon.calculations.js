function GetLatLonfromD(lat1, lon1, lat2, lon2, d)//lat/lon in DD, d in miles
{
    ...
    lat2 = lat2 * factor;
    lon2 = lon2 * factor;
    var dc = dconv(3); /* get distance conversion factor */
    var d23 = d;

    d23 /= dc;   // in nm
    //alert("dc=" +dc)

    var cd1 = crsdist(lat1, lon1, lat2, lon2);  // compute crs and distance (crs = heading)
    var crs12 = cd1.crs12 * (180 / Math.PI);
    var crs21 = cd1.crs21 * (180 / Math.PI);

//    if (crs12 > 0 || crs12 < 90) { crs12 = 45.0; }
//    else if (crs12 >= 90 || crs12 < 180) { crs12 = 135; }
//    else if (crs12 >= 180 || crs12 < 270) { crs12 = 225; }
//    else { crs12 = 315; }
...
//    if (ellipse.name == "Sphere") {
    ...
    var retlat = cd.lat * (180 / Math.PI);
    var retlon = cd.lon * (180 / Math.PI);
    var retlatlon = retlat + "," + retlon;
//    } 
//    else {
//        // elliptic code
