...
function passwordlinkcheck() {
    ...
    var link = href.split("token=");
    $.ajax({
        type: 'POST',
        url: '/LoginUser.svc/CheckPasswordResetLink',
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({ email: username, token: link[1] }),
        dataType: 'json',
...
function PasswordReset_Success(returnObj) {
    var val = returnObj.d;
    if (val[0] == 1) {
...
function PasswordReset_Fail() {
}

function showModalForgotpass() {
    $('#forgotPass_Modal').modal('show');
    $("#forgotPass_Modal").draggable({ handle: ".modal-body" });
}
    ...
    function forgotPassword() {
        ...
        $.ajax({
            type: 'POST',
            url: '/LoginUser.svc/ForgotPassGenerate',
            contentType: 'application/json; charset=utf-8',
        ...
        function Reset_Success(resultObj) {
            var val = resultObj.d[0];
            $("#forgotPasssuccessmessage").empty();
            $("#forgotPasserrormessage").empty();
        
            if (val == 1) {
                $("#forgotPasssuccessmessage").css('display', '');
                $("#forgotPasserrormessage").css('display', 'none');
                $("#forgotPasssuccessmessage").append(resultObj.d[1]);
            }
            else {
                $("#forgotPasserrormessage").css('display', '');
                $("#forgotPasssuccessmessage").css('display', 'none');
                $("#forgotPasserrormessage").append(resultObj.d[1]);
            }
        }
